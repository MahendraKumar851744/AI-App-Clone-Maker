...
<body>
  ...
  <main>
    ...
    <section class="panel">
      <h2>Elements</h2>
      <input id="elementSearch" type="search" placeholder="Search text, ID, class, bounds…">
  ...
  <script>
    ...
    byId("identity").textContent =
      `${{get(DATA, "screen.package")}} · ${{get(DATA, "screen.activity")}} · ${{get(DATA, "captured_at")}}`;
    const badgeValues = [
    ...
    const metrics = {{
      ...
      "Screenshot": `${{get(DATA, "screenshot.width")}}×${{get(DATA, "screenshot.height")}}`,
