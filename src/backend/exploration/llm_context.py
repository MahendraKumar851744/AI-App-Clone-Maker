    ...
    *,
    ...
    if actions:
        ...
        if max_actions and len(actions) > max_actions:
            lines.append(f"- … {len(actions) - max_actions} additional actions omitted")
    else:
    ...
    if visible_text:
        ...
        if max_text_items and len(visible_text) > max_text_items:
            lines.append(
                f"- … {len(visible_text) - max_text_items} additional text items omitted"
            )
    else:
    ...
    if informative:
        ...
        if max_elements and len(informative) > max_elements:
            lines.append(
                f"- … {len(informative) - max_elements} additional elements omitted"
            )
    else:
    ...
    lines.extend(
        [
            ...
            (
                ...
                + (f" — {_clean(json.dumps(errors, ensure_ascii=False))}" if errors else ".")
            ),
            "",
            "## CONTEXT COVERAGE",
            (
