...
from backend.errors import UnsupportedScreenContractError
from backend.exploration.llm_context import (
    build_llm_context,
    context_inventory,
    estimate_tokens,
)
...
class ScreenContextExporter:
    ...
    def _load_stored(self, value: Any) -> tuple[JsonObject, JsonObject]:
        ...
        if set(value) != {"run_id", "screen_id"}:
            raise ContextValidationError(
                "'screen_ref' must contain exactly 'run_id' and 'screen_id'."
            )
        run_id = value.get("run_id")
        screen_id = value.get("screen_id")
        if not self._identifier(run_id) or not self._identifier(screen_id):
            raise ContextValidationError(
                "'run_id' and 'screen_id' must be valid captured identifiers."
            )
        path = (
            self.output_root
            / str(run_id)
