...
from backend.exploration.appium import AppiumExplorer
from backend.exploration.models import JsonObject, ScreenCapture
...
class ExplorationRunNotFoundError(ActionError):
    pass
...
class ActionRequest:
    ...
    def from_dict(cls, payload: JsonObject) -> "ActionRequest":
        ...
        if not screen_id or not RUN_ID_PATTERN.fullmatch(screen_id):
            raise ActionValidationError(
...
@dataclass
class ManagedExploration:
    explorer: AppiumExplorer
    store: ExplorationStore
    lock: threading.Lock = field(default_factory=threading.Lock)


class ExplorationActionManager:
    """Run-scoped live Appium sessions behind one generic action contract."""
    ...
    def __init__(
        ...
        *,
        ...
        self._sessions: dict[str, ManagedExploration] = {}
        self._manager_lock = threading.Lock()
        atexit.register(self.close_all)

    def execute(self, run_id: str, payload: JsonObject) -> JsonObject:
        request = ActionRequest.from_dict(payload)
        expected = self._load_screen(run_id, request.screen_id)
        managed = self._managed_session(run_id, expected)
        with managed.lock:
    ...
    def _managed_session(
        ...
        with self._manager_lock:
            managed = self._sessions.get(run_id)
            if managed is not None:
                return managed
            store = ExplorationStore(self.output_root)
            try:
                store.attach_run(run_id)
    ...
    def _load_screen(self, run_id: str, screen_id: str) -> JsonObject:
        self._validate_run_id(run_id)
        if not RUN_ID_PATTERN.fullmatch(screen_id):
            raise ScreenNotFoundError("Invalid screen identifier.")
        path = self.output_root / run_id / "screens" / screen_id / "appium-result.json"
        if not path.is_file():
            raise ScreenNotFoundError(
                f"Screen '{screen_id}' does not exist in run '{run_id}'."
            )
    ...
    def _validate_run_id(run_id: str) -> None:
        if not RUN_ID_PATTERN.fullmatch(run_id):
            raise ExplorationRunNotFoundError("Invalid exploration run identifier.")
